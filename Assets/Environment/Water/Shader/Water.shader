Shader "Custom/Water"
{
	Properties
	{
		...
		_NormalAlpha("normal alpha",  Range(0, 10)) = 2
	...
	SubShader
	{
		...
		{
			v.vertex.y += sin((abs(v.texcoord.x * 2 - 1) * 3) + sin(_Time.y * 1)) * (_Height /100);
		}
		...
		void surf(Input IN, inout SurfaceOutput o)
		{
			...
			o.Normal = (fNormal1 + fNormal2) / _Speed;
			...
			//grab
			float4 fNoise = tex2D(_MainTex, IN.uv_MainTex + (_Time.x * _Speed));
